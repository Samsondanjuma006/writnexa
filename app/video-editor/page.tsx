...
export default function VideoEditorPage() {
  ...
  async function exportSplitClips() {
    if (!videoFile || !duration) {
      setSplitError("Please upload a video before exporting split clips.");
      return;
    }

    if (trimEnd <= trimStart) {
      setSplitError("The selected trim range is invalid.");
      return;
    }

    const points = splitPoints
      .filter((point) => point > trimStart + 0.05 && point < trimEnd - 0.05)
      .sort((a, b) => a - b);

    if (points.length === 0) {
      setSplitError("Add at least one split point before exporting split clips.");
      return;
    }
    ...
    try {
      ...
      const inputExtension = getFileExtension(videoFile.name);
      const inputName = `split-input${inputExtension}`;

      const boundaries = [trimStart, ...points, trimEnd];

      await ffmpeg.writeFile(inputName, await fetchFile(videoFile));

      const progressHandler = ({ progress }: { progress: number }) => {
        const segmentProgress = Math.min(1, Math.max(0, progress));
        setSplitProgress(
          Math.min(
            99,
            Math.max(
              0,
              Math.round(
                ((Math.min(boundaries.length - 1, Math.floor(segmentProgress * (boundaries.length - 1))) +
                  (segmentProgress * (boundaries.length - 1)) % 1) /
                  (boundaries.length - 1)) *
                  100,
              ),
            ),
      ...
      try {
        for (let index = 0; index < boundaries.length - 1; index += 1) {
          ...
          const outputName = `writnexa-split-${index + 1}.mp4`;
          ...
          if (typeof outputData === "string") {
            throw new Error("FFmpeg returned an invalid split video output.");
          }
          ...
          link.href = outputUrl;
          link.download = `${stripExtension(videoFile.name)}-split-${index + 1}.mp4`;
          document.body.appendChild(link);
          link.click();
          link.remove();
          ...
          setSplitProgress(
            Math.round(((index + 1) / (boundaries.length - 1)) * 100),
          );
        }
      } finally {
        ffmpeg.off("progress", progressHandler);
      ...
      setSplitError(
        error instanceof Error
          ? error.message
          : "Split export failed. Please try again.",
      );
    } finally {
                ...
                <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4">
                  ...
                  {splitPoints.length > 0 && (
                    <>
                      ...
                      <button
                        type="button"
                        onClick={exportSplitClips}
                        disabled={isSplitting || isExporting}
                        className="mt-4 inline-flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-semibold text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50"
                      >
                        <Download className="h-3.5 w-3.5" />
                        {isSplitting
                          ? `Exporting ${splitProgress}%`
                          : "Export split clips"}
                      </button>
                    </>
                  )}
